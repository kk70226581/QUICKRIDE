  ...
  io.on("connection", (socket) => {
    ...
    socket.on("disconnect", async () => {
      ...
      if (connectedUser?.userType === "user") {
        await userModel.findByIdAndUpdate(connectedUser.userId, { $unset: { socketId: "" } });
      } else if (connectedUser?.userType === "captain") {
        await captainModel.findByIdAndUpdate(connectedUser.userId, { $unset: { socketId: "" } });
      }
    });
  });
}
